...
<!--      table表格区域-->
      <el-table :data="goodslist" border stripe>
          ...
          <el-table-column label="商品名称" prop="goods_name" width="550px" align="center">
          </el-table-column>
          <el-table-column label="商品价格(元)" prop="goods_price" width="120px" align="center">
          </el-table-column>
          <el-table-column label="商品重量" prop="goods_weight" width="100px" align="center">
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleSizeChange(id) {
      this.queryInfo.pagesize = id
      this.getGoodsList()
    },
    handleCurrentChange(id) {
      this.queryInfo.pagenum = id
      this.getGoodsList()
    },
    async removeById(row) {
      const e = await this.$confirm('此操作将永久删除该商品, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).catch(err => err)
      // console.log(e)
      if (e !== 'confirm') return this.$message.info('取消删除商品')
